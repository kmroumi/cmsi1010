...
def deal(number_of_hands, cards_per_hand):
    ...
    if number_of_hands <= 0:
        raise ValueError("There must be at least one hand.")
    if cards_per_hand <= 0:
        raise ValueError("Each hand must contain at least one card.")
    if number_of_hands * cards_per_hand > 52:
        raise ValueError(
            "Not enough cards in the deck to deal the requested hands.")
